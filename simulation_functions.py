...
def CalcPXTH(X,STADE,R):
    ...
    P=np.zeros((3,3))
    P[0,0]=-(d-x)/m-(d-x)/h;         P[0,1]=-(s-y)/m+y/h;             P[0,2]=z/m+z/h;
    P[1,0]=-(d-x)/m+x/n    ;         P[1,1]=-(s-y)/m-(s-y)/n;         P[1,2]=z/m+z/n;
    P[2,0]=-(d-x)/m+x/n+x/k-(d-x)/h; P[2,1]=-(s-y)/m-(s-y)/n+y/k+y/h; P[2,2]=z/m+z/n+z/k+z/h;
...
def CalcHXTH(X,STADE,R):
    ...
    H1=np.zeros((3,3))
    H1[0,0]=1/m+1/h-((d-x)**2)/m**3-((d-x)**2)/h**3;    H1[0,1]=-(d-x)*(s-y)/m**3+(d-x)*y/h**3;          H1[0,2]=(d-x)*z/m**3+(d-x)*z/h**3;
    H1[1,0]=-(d-x)*(s-y)/m**3+(d-x)*y/h**3;             H1[1,1]=1/m+1/h-((s-y)**2)/m**3-(y**2)/h**3;     H1[1,2]=(s-y)*z/m**3-y*z/h**3;
    H1[2,0]=(d-x)*z/m**3+(d-x)*z/h**3;                  H1[2,1]=(s-y)*z/m**3-y*z/h**3;                   H1[2,2]=1/m+1/h-(z**2)/m**3-(z**2)/h**3;
    
    H2=np.zeros((3,3))
    H2[0,0]=1/m+1/n-((d-x)**2)/m**3-(x**2)/n**3;        H2[0,1]=-(d-x)*(s-y)/m**3+x*(s-y)/n**3;          H2[0,2]=(d-x)*z/m**3-x*z/n**3;
    H2[1,0]=-(d-x)*(s-y)/m**3+x*(s-y)/n**3;             H2[1,1]=1/m+1/n-((s-y)**2)/m**3-((s-y)**2)/n**3; H2[1,2]=(s-y)*z/m**3+(s-y)*z/n**3;
    H2[2,0]=(d-x)*z/m**3-x*z/n**3;                      H2[2,1]=(s-y)*z/m**3+(s-y)*z/n**3;               H2[2,2]=1/m+1/n-(z**2)/m**3-(z**2)/n**3;
    
    H3=np.zeros((3,3))
    H3[0,0]=1/m+1/n+1/k+1/h-((d-x)**2)/m**3-(x**2)/n**3-(x**2)/k**3-((d-x)**2)/h**3;    H3[0,1]=-(d-x)*(s-y)/m**3+x*(s-y)/n**3-x*y/k**3+(d-x)*y/h**3;                     H3[0,2]=(d-x)*z/m**3-x*z/n**3-x*z/k**3+(d-x)*z/h**3;
    H3[1,0]=-(d-x)*(s-y)/m**3+x*(s-y)/n**3-x*y/k**3+(d-x)*y/h**3;                       H3[1,1]=1/m+1/n+1/k+1/h-((s-y)**2)/m**3-((s-y)**2)/n**3-(y**2)/k**3-(y**2)/h**3;  H3[1,2]=(s-y)*z/m**3+(s-y)*z/n**3-y*z/k**3-y*z/h**3;
    H3[2,0]=(d-x)*z/m**3-x*z/n**3-x*z/k**3+(d-x)*z/h**3;                                H3[2,1]=(s-y)*z/m**3+(s-y)*z/n**3-y*z/k**3-y*z/h**3;                              H3[2,2]=1/m+1/n+1/k+1/h-(z**2)/m**3-(z**2)/n**3-(z**2)/k**3-(z**2)/h**3;
...
def EqDiff(Y,t,STADE,R,mA,Sinv,L,G,g,Kpos,Kvit,time,vitXref,posXref,vitThref,posThref,Fpert):
    ...
    A=np.array([[np.dot(np.dot(Xpt.transpose(),H1),Xpt)[0][0]],[np.dot(np.dot(Xpt.transpose(),H2),Xpt)[0][0]],[np.dot(np.dot(Xpt.transpose(),H3),Xpt)[0][0]]])
    B=np.dot(L,Thpt)
    C=Kpos*(Th0-Th)
    D=Kvit*(Thpt0-Thpt)
